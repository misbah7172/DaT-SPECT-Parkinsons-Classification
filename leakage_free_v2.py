...
for seed in [42]:
    for train_idx, test_idx in StratifiedGroupKFold(5, shuffle=True, random_state=seed).split(X1, y, groups):
        ...
        ridge_d = ridge.decision_function(X1_test_s)
        ridge_prob = np.expit(ridge_d / 0.8958)
...
print("Config A (n_sel=58, C=1.312, alpha=8.108):")
print(f"  Mean OOF AUROC: {np.mean(aucs):.4f} ± {np.std(aucs):.4f}")
print(f"  Mean OOF LogLoss: {np.mean(lls):.4f} ± {np.std(lls):.4f}")
