"""Deep 3D variant B: strided-conv downsampling (no maxpool), wider channels,
2 extra ResBlocks. Registered crops, Inorm, TTA translations, save per-seed OOF.
Run detached; resume-capable via progress file."""
import os, time
import numpy as np
...
SEEDS = [555, 888, 123, 321]
EPOCHS = 35
BATCH = 8
device = 'cuda' if torch.cuda.is_available() else 'cpu'
PROG = os.path.join(OUT, 'cnn3dr3_progress.txt')
...
def main():
    ...
    for seed in SEEDS:
        ...
        with open(PROG, 'a') as f:
            f.write(str(seed) + '\n')
        np.save(os.path.join(OUT, f'cnn3dr3_seed{seed}.npy'), oof_seed)
        print(f'saved cnn3dr3_seed{seed}', flush=True)
