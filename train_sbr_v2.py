"""Add voxel spacing features to existing SBR and retrain models."""
...
def add_spacing_features():
    ...
    sbr = sbr.drop(columns=['label'], errors='ignore')
    sbr = sbr.set_index('uid').loc[labels.index]
    merged = labels.join(site).join(geom)
    ...
    sbr['voxel_side'] = merged['voxel_side'].values
    sbr['voxel_vol'] = merged['voxel_vol'].values
    sbr['log_voxel_vol'] = np.log(merged['voxel_vol'].values)
    sbr['voxel_side_sq'] = merged['voxel_side'].values ** 2
    sbr['is_highres'] = (merged['voxel_side'].values < 2.0).astype(float)
    sbr['is_lowres'] = (merged['voxel_side'].values > 3.5).astype(float)
    cols = [c for c in sbr.columns]
    a = sbr[cols].values.astype(np.float64)
...
if __name__ == '__main__':
    if not os.path.exists(LOG):
        open(LOG, 'w').close()
    log('=== SBR v2 (spacing-enhanced) training ===')
    X, y, groups, cols = add_spacing_features()
    log(f'Features: {X.shape[1]} cols from {len(cols)} base features')
    models = {
    ...
    log('=== Fitting final models on all data ===')
    sc = StandardScaler()
    ...
    for name, factory in models.items():
        ...
        log(f'  Saved {name}.pkl (overwriting)')
    pickle.dump(sc, open(os.path.join(OUT_W, 'sbr_full_scaler.pkl'), 'wb'))
    json.dump(cols + ['voxel_side', 'voxel_vol', 'log_voxel_vol', 'voxel_side_sq', 'is_highres', 'is_lowres',
                       'lr_put_asym', 'lr_put_ratio', 'lr_put_sum', 'lr_put_diff',
                       'lr_cau_asym', 'lr_cau_ratio', 'lr_cau_sum',
                       'lp_post_ant', 'rp_post_ant', 'lp_cr', 'rp_cr', 'total_str', 'lr_total_asym'],
              open(os.path.join(OUT_W, 'sbr_full_cols.json'), 'w'))
