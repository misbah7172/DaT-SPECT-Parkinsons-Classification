...
for n in [44, 58, 80]:
    ...
    # Simple blend: weights concept from v6
    # lr 0.081, et 0.209, hgb 0.216, rf 0.0, ridge 0.052
    # But use full X1 (not X1_s) for models that need all features, 
    # and X1_s for lr/et/hgb which use selected features
    
    # Actually, let me just evaluate on X1_s for all models for fair comparison
    ...
    # rf on selected features (weight 0.0 in v6, so omit or use small weight)
    preds += rf.predict_proba(X1_s)[:, 1] * 0.0
    # ridge on all features (but we'll use X1, not X1_s)
    # Actually let's just use X1 for ridge since ridge was trained on all features in v6
    # But for consistency, use X1_s for all... let me just use X1_s for all
    # ridge on X1_s
    ridge = RidgeClassifier(alpha=1.0, random_state=seed).fit(X1_s, y)
...
print()
print("=== Ridge alpha SWEEP (with n_sel=58, using X1 not X1_s) ===")
# Ridge alone on full X1
...
print()
print("=== LR C SWEEP (n_sel=58, ridge alpha=1.0, using X1) ===")
