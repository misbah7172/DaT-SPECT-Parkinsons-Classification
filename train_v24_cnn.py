"""V24 CNN training: 3 Net3dR + 3 Net3dBig on 2.0mm/80^3 volumes."""
import os
import sys
import time
import json
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            try:
                ...
                auc = 0.0
            print(f"  Epoch {epoch}: val_loss={avg_val_loss:.4f} best={best_val_loss:.4f} auc={auc:.4f} pat={patience_counter}")
...
def main():
    ...
    print(f"Available volumes: {len(labels_df)}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(labels_arr, labels_arr, groups)):
        print(f"\n{'='*60} FOLD {fold} {'='*60}")
        ...
        for model_idx, (arch, name, seed) in enumerate(zip(archs, model_names, model_seeds)):
            ...
            print(f"    Done: val_loss={val_loss:.4f}, time={elapsed:.0f}s")
        ...
        # Mark oof_mask for this fold
        oof_mask[val_idx] = True

        # Evaluate fold (average across all 6 models)
        ...
        fold_metrics.append({"fold": fold, "logloss": float(fold_ll), "auc": float(fold_auc)})
