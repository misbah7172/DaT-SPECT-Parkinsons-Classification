"""V24 CNN training: folds 0-2 with z-score normalization."""
...
def train_model(model, train_loader, val_loader, device, epochs=80, lr=1e-4):
    ...
    best_vl = float("inf"); best_state = None; pat = 0; warmup = False
    ...
    for epoch in range(epochs):
        ...
        if warmup and avg < best_vl:
            best_vl = avg; best_state = {k:v.clone() for k,v in model.state_dict().items()}; pat = 0
        elif warmup: pat += 1
        if (epoch+1)%10==0 or epoch==0:
            try: auc = roc_auc_score(np.array(vt), np.array(vp))
            except: auc = 0
            print(f"  Ep {epoch}: loss={avg:.4f} best={best_vl:.4f} auc={auc:.4f} pat={pat}", flush=True)
        if pat >= 15:
...
def main():
    ...
    for fold in range(3):  # folds 0,1,2
