"""Resume-capable deep 3D CNN with mixed precision, launched detached.
Saves per-seed OOF; skips completed seeds via progress file."""
import os, time
import numpy as np
...
def main():
    ...
    for seed in SEEDS:
        if str(seed) in done:
            print('skip seed', seed, flush=True); continue
        oof_seed = np.zeros(n)
