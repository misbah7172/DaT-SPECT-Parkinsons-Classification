"""
V9: Use XGBoost, LightGBM, CatBoost + sklearn models for stronger OOF.
Target: AUC > 0.93, LL < 0.25
"""
import json
import os
import shutil
...
def main():
    ...
    active = [(names[i], float(best_w[i])) for i in range(len(names)) if best_mask[i]]
    for n, wt in sorted(active, key=lambda x: -x[1]):
        print(f"  {n:20s} w={wt:.3f}")
...
if __name__ == "__main__":
    ...
    for si, seed in enumerate(seeds):
        for xi, X in enumerate(Xs):
            ...
            n_feat = Xs_all.shape[1]
            mi_top = min(44, n_feat)
            mi = mutual_info_classif(Xs_all, y, random_state=seed)
            sel = np.argsort(mi)[::-1][:mi_top]
            Xt = Xs_all[:, sel]
            ...
            joblib.dump(xgb_m, os.path.join(out_dir, f"weights/s{si}_x{xi}_xgb.pkl"))
            joblib.dump(lgb_m, os.path.join(out_dir, f"weights/s{si}_x{xi}_lgb.pkl"))
            joblib.dump(cb_m, os.path.join(out_dir, f"weights/s{si}_x{xi}_cb.pkl"))
    ...
    ds_map = {}
    for xi, ds_name in enumerate(["sbr", "phys", "morph", "atlas"][:n_datasets]):
        for mi_idx, mname in enumerate(model_names):
            stream_idx = xi * len(model_names) + mi_idx
            ds_map[str(stream_idx)] = {"dataset": ds_name, "model": mname}

    cfg = {
        "n_seeds": len(seeds),
        "n_datasets": n_datasets,
        "dataset_names": ["sbr", "phys", "morph", "atlas"][:n_datasets],
