...
def main():
    ...
    n_total = len(order)
    probas = []
    iso = joblib.load(os.path.join(weights_dir, "isotonic.pkl"))
    for idx, uid in enumerate(order):
        ...
        p = expit(logit(np.clip(p, 1e-6, 1 - 1e-6)) / cfg["temperature"])
        p = float(cfg["isotonic"].predict(np.array([[p]]))[0])
...
def main():
    ...
    for si, seed in enumerate(seeds):
        for xi in range(2):
            ...
            for tr, va in StratifiedGroupKFold(N_FOLDS, shuffle=True, random_state=seed).split(X, y, groups):
                ...
                ridge = RidgeClassifier(alpha=1.0, random_state=seed).fit(Xtr_s, y[tr])
                oof_seed.setdefault((si, xi, "ridge"), np.zeros(len(y)))[va] = ridge.decision_function(Xva_s)
                ...
                oof_seed.setdefault((si, xi, "et"), np.zeros(len(y)))[va] = et.predict_proba(Xva_t)[:, 1]
                hgb = HistGradientBoostingClassifier(max_iter=400, max_depth=5, learning_rate=0.02,
                                                     l2_regularization=1.0, min_samples_leaf=10,
                                                     random_state=seed).fit(Xtr_t, y[tr])
                oof_seed.setdefault((si, xi, "hgb"), np.zeros(len(y)))[va] = hgb.predict_proba(Xva_t)[:, 1]
                rf = RandomForestClassifier(500, max_depth=10, n_jobs=-1, random_state=seed).fit(Xtr_t, y[tr])
                oof_seed.setdefault((si, xi, "rf"), np.zeros(len(y)))[va] = rf.predict_proba(Xva_t)[:, 1]
    ...
    iso = IsotonicRegression(out_of_bounds="clip").fit(p_temp, y)
    final = np.clip(iso.predict(p_temp), 0.005, 0.995)
    ...
    print("temp", round(temp, 4), "temp LL", round(log_loss(y, np.clip(p_temp, 0.005, 0.995)), 4))
    print("FINAL (temp+iso) AUC", round(roc_auc_score(y, final), 4), "LL", round(log_loss(y, final), 4))
    ...
    for si, seed in enumerate(seeds):
        for xi in range(2):
            ...
            ridge = RidgeClassifier(alpha=1.0, random_state=seed).fit(Xs_all, y)
            et = ExtraTreesClassifier(500, max_depth=12, n_jobs=-1, random_state=seed).fit(Xs_all[:, sel], y)
            hgb = HistGradientBoostingClassifier(max_iter=400, max_depth=5, learning_rate=0.02,
                                                 l2_regularization=1.0, min_samples_leaf=10,
                                                 random_state=seed).fit(Xs_all[:, sel], y)
            rf = RandomForestClassifier(500, max_depth=10, n_jobs=-1, random_state=seed).fit(Xs_all[:, sel], y)
    ...
    joblib.dump(iso, os.path.join(out_dir, "weights/isotonic.pkl"))
    ...
    with open(os.path.join(out_dir, "oof_metrics.json"), "w") as f:
        json.dump({"oof_auroc": round(float(roc_auc_score(y, final)), 4),
                   ...
                   "temp_ll": round(float(log_loss(y, np.clip(p_temp, 0.005, 0.995))), 4)}, f, indent=2)
    print("saved", out_dir)
