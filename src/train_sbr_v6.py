...
def main():
    ...
    for si, seed in enumerate(seeds):
        for xi in range(2):
            ...
            for tr, va in StratifiedGroupKFold(N_FOLDS, shuffle=True, random_state=seed).split(X, y, groups):
                ...
                lr = LogisticRegression(C=LR_C, max_iter=3000, random_state=seed).fit(Xtr_s, y[tr])
                oof_seed.setdefault((si, xi, "lr"), np.zeros(len(y)))[va] = lr.predict_proba(Xva_s)[:, 1]
                ridge = RidgeClassifier(alpha=RIDGE_ALPHA, random_state=seed).fit(Xtr_s, y[tr])
    ...
    print("temp", round(temp, 4), "temp LL", round(log_loss(y, final), 4))
    ...
    for si, seed in enumerate(seeds):
        for xi in range(2):
            ...
            sel = np.argsort(mi)[::-1][:MI_TOP]
            lr = LogisticRegression(C=LR_C, max_iter=3000, random_state=seed).fit(Xs_all, y)
            ridge = RidgeClassifier(alpha=RIDGE_ALPHA, random_state=seed).fit(Xs_all, y)
            et = ExtraTreesClassifier(700, max_depth=9, n_jobs=-1, random_state=seed).fit(Xs_all[:, sel], y)
            hgb = HistGradientBoostingClassifier(max_iter=500, max_depth=4, learning_rate=0.0489,
                                                 l2_regularization=4.6, min_samples_leaf=10,
                                                 random_state=seed).fit(Xs_all[:, sel], y)
            rf = RandomForestClassifier(300, max_depth=14, n_jobs=-1, random_state=seed).fit(Xs_all[:, sel], y)
