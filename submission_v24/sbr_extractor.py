"""V24 SBR feature extractor with PVE intensity correction.

Changes from v23:
- PVE intensity correction: corrects SBR values for scanner-resolution-dependent
  partial volume effect dilution, not just volume features
- Intensity correction uses a resolution-dependent recovery coefficient
- Based on the principle that smaller structures (putamen) in coarser scans
  have systematically lower measured SBR due to blurring
"""
...
# Reference voxel volume (2.5mm isotropic = 15.625 mm³)
PVE_REF_VOLUME = 15.625

# Recovery coefficients for PVE intensity correction
# Based on: RC(v) ≈ 1 / (1 + k * (v_ref/v)^(1/3)) where k controls correction strength
# These are approximate — should be calibrated on phantom data if available
PVE_INTENSITY_K = 0.3  # correction strength (tunable)
...
def _pve_intensity_correction(sbr_value, voxel_volume, ref_volume=PVE_REF_VOLUME, k=PVE_INTENSITY_K):
    """Correct SBR intensity for partial volume effect.

    In coarser-resolution scans, small structures like the putamen have
    systematically lower measured SBR due to blurring toward background.
    This correction applies a recovery coefficient to compensate.

    Args:
        sbr_value: measured SBR value
        voxel_volume: actual voxel volume in mm³
        ref_volume: reference voxel volume (2.5mm isotropic)
        k: correction strength (higher = stronger correction)

    Returns:
        Corrected SBR value
    """
    if voxel_volume <= 0 or ref_volume <= 0:
        return sbr_value

    # Recovery coefficient: coarser scans need more correction
    # RC = 1 / (1 + k * (v_ref/v)^(1/3))
    ratio = (ref_volume / voxel_volume) ** (1.0 / 3.0)
    rc = 1.0 / (1.0 + k * ratio)

    # Apply correction: divide by recovery coefficient
    # (RC < 1 for coarser scans, so correction increases SBR)
    return sbr_value / max(rc, 0.5)  # floor at 0.5 to prevent overcorrection


def extract_features(nii_path, voxel_volume=None):
    """Extract SBR features with PVE intensity correction.

    Args:
        nii_path: path to NIfTI file
        voxel_volume: voxel volume in mm³ (if None, computed from header)
    """
    ...
    def region_sbr(hside_p, y_pro, cx=None, cyy=None):
        ...
        raw_sbr = _sbr(sm[x[m], y[m], z[m]], bg)
        # Apply PVE intensity correction
        return _pve_intensity_correction(raw_sbr, voxel_volume)
    ...
    def region_max_sbr(hside_p, y_pro):
        ...
        raw_max = (sm[x[m], y[m], z[m]].max() - bg) / bg
        return _pve_intensity_correction(raw_max, voxel_volume)

    def region_med_sbr(hside_p, y_pro):
        m = region_mask(hside_p, y_pro, hx, cy)
        raw_med = (np.median(sm[x[m], y[m], z[m]]) - bg) / bg
        return _pve_intensity_correction(raw_med, voxel_volume)
    ...
    def ant_post_sbr(hside_p):
        ...
        ant = py >= med_y
        post = py < med_y
        s_ant = _pve_intensity_correction(_sbr(sm[px[ant], py[ant], pz[ant]], bg), voxel_volume)
        s_post = _pve_intensity_correction(_sbr(sm[px[post], py[post], pz[post]], bg), voxel_volume)
        return s_ant, s_post
    ...
    # Store voxel volume for downstream use
    feat["voxel_volume"] = voxel_volume
    ...
    for q in (95, 97, 99):
        ...
        for name, hs in (("lp", True), ("rp", False)):
            ...
            put = m & (yq < cyq)
            cau = m & (yq >= cyq)
            raw_sbr = _sbr(sm[xq[put], yq[put], zq[put]], bg)
            feat[f"sbr_{name}_p{q}"] = _pve_intensity_correction(raw_sbr, voxel_volume)
            feat[f"vol_{name}_p{q}"] = int(put.sum())
        for name, hs in (("lc", True), ("rc", False)):
            m = (xq > hx) if hs else (xq <= hx)
            put = m & (yq < cyq)
            cau = m & (yq >= cyq)
            raw_sbr = _sbr(sm[xq[cau], yq[cau], zq[cau]], bg)
            feat[f"sbr_{name}_p{q}"] = _pve_intensity_correction(raw_sbr, voxel_volume)
    ...
    for s in (0.5, 2.0):
        ...
        for name, hs, yp in (("lp", True, False), ("rp", False, False),
            ...
            raw_sbr = _sbr(sms[x[m], y[m], z[m]], bg)
            feat[f"sbr_{name}_{tag}"] = _pve_intensity_correction(raw_sbr, voxel_volume)
