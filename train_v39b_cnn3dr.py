"""Resume-capable deep 3D CNN trainer. Runs seeds from SEEDS; skips already-completed
seeds by tracking progress file. Designed to run under Start-Process (no shell kill)."""
import os, time, sys
import numpy as np
...
SEEDS = [42, 777, 2024, 100, 200, 300]
EPOCHS = 45
BATCH = 12
device = 'cuda' if torch.cuda.is_available() else 'cpu'
PROG = os.path.join(OUT, 'cnn3dr2_progress.txt')
...
def main():
    ...
    for seed in SEEDS:
        ...
        with open(PROG, 'a') as f:
            f.write(str(seed) + '\n')
        np.save(os.path.join(OUT, f'cnn3dr2_seed{seed}.npy'), oof_seed)
        print(f'saved cnn3dr2_seed{seed}', flush=True)
