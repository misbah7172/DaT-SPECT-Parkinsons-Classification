"""Stronger 2.5D CNN: wider/deeper (residual blocks), 8 seeds, TTA, fixed atlas window."""
import os, time
import numpy as np
...
print('input = REG' if 'REG' in os.environ else 'input = INORM', flush=True)
...
print(f'2.5D-strong OOF: AUC={roc_auc_score(y, oof):.4f} LL={log_loss(y, np.clip(oof,1e-6,1-1e-6)):.4f}')
print(f'2.5D-strong SC:  AUC={roc_auc_score(y, oof_sc):.4f} LL={log_loss(y, oof_sc):.4f}')
name = 'cnn25r2_oof' if 'REG' in os.environ else 'cnn25s_oof'
name_sc = 'cnn25r2_oof_sc' if 'REG' in os.environ else 'cnn25s_oof_sc'
np.save(os.path.join(OUT, name), oof)
np.save(os.path.join(OUT, name_sc), oof_sc)
print('saved', name, 'in %.0fs' % (time.time()-t_start))