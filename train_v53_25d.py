"""V53: 2.5D approach using inflated ImageNet weights (2D->3D via slice aggregation)
Uses torchvision ResNet18 pretrained on ImageNet, modified for 1-channel input,
applied slice-wise, features aggregated over slices, then small MLP head.
Protocol: same SGKF42 split, 40 epochs, early stop patience 16, cosine LR, AMP.
"""
import os, time
import numpy as np
...
DATA = r'E:\DaT\cnn3d'
OUT = r'E:\DaT\submission_v26\weights_v53'  # separate dir to avoid overwriting v52
os.makedirs(OUT, exist_ok=True)
LOG = os.path.join(r'\DaT\submission_v26', 'train_v53.log')
...
def inflate_conv2d_to_3d(conv2d, depth_repeat=4):
    """Inflate a 2D conv layer to 3D by repeating kernels along depth axis.
    Args:
        conv2d: nn.Conv2d layer
        depth_repeat: number of times to repeat the kernel along depth
    Returns:
        nn.Conv3d layer with inflated weights
    """
    # Get 2D kernel shape: [out_channels, in_channels, kH, kW]
    weight_2d = conv2d.weight.data  # [oc, ic, kH, kW]
    # Repeat along depth: [oc, ic, depth_repeat, kH, kW]
    weight_3d = weight_2d.unsqueeze(2).repeat(1, 1, depth_repeat, 1, 1)
    # Scale by depth_repeat to keep approximately the same output magnitude
    weight_3d = weight_3d / depth_repeat
    
    # Create new 3D conv layer
    conv3d = nn.Conv3d(
        in_channels=conv2d.in_channels,
        out_channels=conv2d.out_channels,
        kernel_size=(depth_repeat, conv2d.kernel_size[0], conv2d.kernel_size[1]),
        stride=(conv2d.stride[0], conv2d.stride[1], conv2d.stride[1]),  # assume stride same in H/W, depth stride=1?
        padding=(depth_repeat//2, conv2d.padding[0], conv2d.padding[1]),
        dilation=(1, conv2d.dilation[0], conv2d.dilation[1]),
...
class SliceWrapper(nn.Module):
    """Wrap a 2D model to process 3D volumes slice-wise and aggregate features."""
    def __init__(self, 
...
def train_fold(arch, seed, fold, tr, va):
    ...
    Xt = torch.from_numpy(X[tr]).float().unsqueeze(1)  # [N,1,D,H,W]
