"""
V14: Focus on aggressive probability calibration.
Paper says: "AUROC > 0.93 but LL > 0.25 indicates overconfident probabilities."
Key: Use Platt scaling, isotonic regression, and combined calibration.
"""
import json
import os
import shutil
...
def optimize_platt_scalar(oof, y):
    """Simple Platt: find best a, b to minimize log loss."""
...
def main():
    ...
    ll_shrunk = log_loss(y, np.clip(p_shrunk, 1e-6, 1-1e-6))
    print(f"SHRINK (a={alpha:.4f}): AUC={roc_auc_score(y, p_shrunk):.4f}  LL={ll_shrunk:.4f}")

    # 4. Platt + shrinkage
    p_platt_shrunk = (1 - alpha) * p_platt + alpha * prior
    ll_platt_shrunk = log_loss(y, np.clip(p_platt_shrunk, 1e-6, 1-1e-6))
    print(f"PLATT+SHRINK: AUC={roc_auc_score(y, p_platt_shrunk):.4f}  LL={ll_platt_shrunk:.4f}")

    # 5. Temperature + shrinkage
    p_temp_shrunk = (1 - alpha) * p_temp + alpha * prior
    ll_temp_shrunk = log_loss(y, np.clip(p_temp_shrunk, 1e-6, 1-1e-6))
    print(f"TEMP+SHRINK: AUC={roc_auc_score(y, p_temp_shrunk):.4f}  LL={ll_temp_shrunk:.4f}")

    # 6. Aggressive shrinkage (more toward prior)
    for agg_alpha in [0.1, 0.15, 0.2, 0.25, 0.3]:
        p_agg = (1 - agg_alpha) * blend + agg_alpha * prior
        ll_agg = log_loss(y, np.clip(p_agg, 1e-6, 1-1e-6))
        print(f"AGG_SHRINK (a={agg_alpha}): AUC={roc_auc_score(y, p_agg):.4f}  LL={ll_agg:.4f}")
    ...
    candidates = {
        ...
        "platt": p_platt,
        "shrink": p_shrunk,
        "platt_shrink": p_platt_shrunk,
        "temp_shrink": p_temp_shrunk,
    }
    
    # Add aggressive shrinkage candidates
    for agg_alpha in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]:
        p_agg = (1 - agg_alpha) * blend + agg_alpha * prior
        candidates[f"shrink_{agg_alpha}"] = p_agg
    ...
    for _ in range(M.shape[1]):
        ...
        for j in range(M.shape[1]):
            ...
            # Try best calibration method
            for mname, p in [("blend", bc), ("temp", expit(logit(np.clip(bc, 1e-6, 1 - 1e-6)) / optimize_temperature(bc, y)))]:
    ...
    return merged, y, groups, Xs, best_w_elim, best_mask, names, best_auc_elim, best_ll_elim, best_method_elim


if __name__ == "__main__":
    merged, y, groups, Xs, best_w, best_mask, names, final_auc, final_ll, best_method = main()

    seeds = SEEDS
    out_dir = "submission_v14"
    ...
    # Recompute best temperature on final blend
    final_blend = np.zeros(len(y))
    for si, seed in enumerate(seeds):
        for xi, X in enumerate(Xs):
            sc = joblib.load(os.path.join(out_dir, f"weights/s{si}_ds{xi}_scaler.pkl"))
            sel = joblib.load(os.path.join(out_dir, f"weights/s{si}_ds{xi}_mi.pkl"))
            Xs_scaled = sc.transform(X)
            Xt = Xs_scaled[:, sel]
            
            base = xi * len(model_names_base)
            for mi_idx, mname in enumerate(model_names_base):
                model = joblib.load(os.path.join(out_dir, f"weights/s{si}_{names[base+mi_idx]}.pkl"))
                if mname in ("lr", "ridge"):
                    if mname == "lr":
                        pred = model.predict_proba(Xs_scaled)[:, 1]
                    else:
                        pred = expit(model.decision_function(Xs_scaled))
                else:
                    pred = model.predict_proba(Xt)[:, 1]
                # Will average later
    
    # Just save config for now
    temp = optimize_temperature(M @ best_w, y)
