"""V42: HONEST global calibration (Temp + Isotonic), fit on outer-train only.
Trains 3 seeds, averages. Uses greedy K=16 selection like v40."""
...
def make_iso():
    """Isotonic fit on outer-train blend (pt) -> yt; apply to pv."""
    def cal(pt, pv, yt, yv, gt, gv):
        iso = IsotonicRegression(out_of_bounds='clip', y_min=eps, y_max=1 - eps)
        iso.fit(pt, yt)
        return iso.predict(pv)
    return cal


def make_temp():
    """Temperature scaling in logit space, fit on outer-train."""
    def cal(pt, pv, yt, yv, gt, gv):
        ...
        T = r.x[0]
        return expit(logit(np.clip(pv, eps, 1 - eps)) / T)
    return cal
...
if __name__ == '__main__':
    ...
    SEEDS = [int(x) for x in sys.argv[3].split(',')] if len(sys.argv) > 3 else [42, 777, 2024]
    cal = {'iso': make_iso, 'temp': make_temp}[calname]()
    for s in SEEDS:
        f = rf'E:\DaT\v30_oof\v42_{calname}_seed{s}.npy'
        if os.path.exists(f):
            log(f'skip seed {s}')
            continue
        log(f'== {calname} seed {s} start')
        o = run(K, s, cal)
        np.save(f, o)
        log(f'== {calname} seed {s}: AUC={roc_auc_score(y, o):.4f} LL={log_loss(y, np.clip(o, eps, 1 - eps)):.4f} saved')
    o = np.zeros(n)
    for s in SEEDS:
        o += np.load(rf'E:\DaT\v30_oof\v42_{calname}_seed{s}.npy')
    o /= len(SEEDS)
    log(f'FINAL {calname}: AUC={roc_auc_score(y, o):.4f} LL={log_loss(y, np.clip(o, eps, 1 - eps)):.4f}')
    ...
    np.save(rf'E:\DaT\v42_{calname}_honest_oof.npy', o)
    log(f'saved v42_{calname}_honest_oof.npy')