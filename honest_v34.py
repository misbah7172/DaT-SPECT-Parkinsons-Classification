"""V34: honest nested with greedy selection over TOP-30 candidate streams (pre-filtered
by train-fold AUC for speed) + per-site calibration fit only on train.
"""
import os, pickle
import numpy as np
...
eps = 1e-6
S = np.hstack([oof_a, oof_b, oof_c, blend_d.reshape(-1,1), oof_roi, cnn3d.reshape(-1,1), cnn25b.reshape(-1,1), cnn25s.reshape(-1,1), cnn25r2.reshape(-1,1), cnn3dr.reshape(-1,1), cnn25v2.reshape(-1,1)])
n, p = S.shape
print('streams:', p, 'n:', n)
...
for K in [16]:
    oof, oof_sc = run(K, 42)
    oof2, oof_sc2 = run(K, 777)
    oof3, oof_sc3 = run(K, 2024)
    o = (oof + oof2 + oof3) / 3
    os_ = (oof_sc + oof_sc2 + oof_sc3) / 3
    print(f'K={K}: AUC={roc_auc_score(y, o):.4f} LL={log_loss(y, np.clip(o,eps,1-eps)):.4f} | '
          f'SC AUC={roc_auc_score(y, os_):.4f} LL={log_loss(y, os_):.4f}', flush=True)
o_final = os_
for site in np.unique(groups):
    m = groups == site
    if m.sum() < 20 or len(np.unique(y[m])) < 2: continue
    print(f'  {site}: n={m.sum()} AUC={roc_auc_score(y[m], o_final[m]):.4f} LL={log_loss(y[m], np.clip(o_final[m],eps,1-eps)):.4f}')
np.save(r'E:\DaT\v34_honest_oof.npy', o_final)
